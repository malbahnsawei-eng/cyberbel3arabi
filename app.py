...
def publish_to_facebook_page(
  ...
  if not page_id or not page_access_token:
    return {
        "status": "error",
        "message": (
            "من فضلك أدخل Page ID و Page Access Token في القائمة الجانبية"
            " أولاً."
        ),
    }
...
def publish_to_instagram(image_url, caption, ig_user_id, page_access_token):
  if not ig_user_id or not page_access_token:
    return {
        "status": "error",
        "message": (
            "من فضلك أدخل Instagram Business Account ID و Access Token."
        ),
    }
  try:
    container_url = (
    ...
    if "id" not in container_data:
      return {
          "status": "error",
          "message": container_data.get("error", {}).get(
              "message", "فشل إنشاء الحاوية"
          ),
      }
    ...
    if "id" in publish_data:
      return {
          "status": "success",
          "message": f"تم النشر بنجاح على إنستجرام! ID: {publish_data['id']}",
      }
    return {
        "status": "error",
        "message": publish_data.get("error", {}).get("message", "فشل النشر"),
    }
  except Exception as e:
    return {"status": "error", "message": str(e)}
        ...
        if not articles:
          ...
          for art in articles:
            ...
            if not is_relevant_for_public(
              ...
              log_automation(
                  f"تم تجاهل (غير مناسب للعامة): {art['title']}", "info"
              )
              continue
            ...
            if pub["status"] == "success":
              log_automation(
                  f"✅ تم النشر تلقائيًا: {art['title']}", "success"
              )
            else:
              log_automation(
                  f"⚠️ فشل نشر '{art['title']}': {pub['message']}", "error"
              )
          ...
          if not posted:
            log_automation(
                "لا توجد مواضيع جديدة غير مكررة ومناسبة للنشر اليوم.", "warning"
            )
      except Exception as e:
        log_automation(
            f"خطأ غير متوقع في التشغيل التلقائي: {str(e)}", "error"
        )
...
def start_scheduler(
  ...
  thread.start()
  log_automation(
      f"تم تفعيل الجدولة التلقائية: يوميًا الساعة {daily_time_display}، من"
      f" {format_date_arabic(start_date_str)} إلى"
      f" {format_date_arabic(end_date_str)}.",
      "info",
  )
  return True
...
with st.sidebar:
  ...
  api_key = st.text_input(
      "Groq API Key",
      type="password",
      help="أدخل مفتاح Groq API من console.groq.com",
  )
...
with tabs[0]:
  ...
  with col_in1:
    url_input = st.text_input(
        "🔗 رابط مقال / خبر توعوي (URL):",
        placeholder="https://example.com/security-article",
    )
    topic_input = st.text_area(
        "📝 أو اكتب الموضوع مباشرة:",
        placeholder="مثال: مخاطر الهجمات عبر الهندسة الاجتماعية...",
    )
  with col_in2:
  ...
  if check_btn:
    if not url_input and not topic_input:
      ...
      if dup:
        st.error(
            f"⚠️ تحذير: هذا الرابط موجود مسبقاً في الأرشيف! (ID: #{dup[0]})"
        )
      else:
  ...
  if generate_btn:
    if not api_key:
      ...
      if enable_deduplication and url_input.strip():
        ...
        if dup:
          st.error(
              "⚠️ تم إيقاف التوليد: هذا الرابط مسجل مسبقاً في الأرشيف (ID:"
              f" #{dup[0]})."
          )
          proceed = False
      ...
      if proceed:
        with st.spinner(
            "🔄 جاري تحليل المقال وتوليد المحتوى والتصميم عبر نموذج"
            f" ({model_choice})..."
        ):
          if url_input.strip():
          ...
          if gen["success"]:
            save_post(
                url_input,
                topic_input if topic_input else "مقال من رابط",
                gen["raw_text"],
            )
  ...
  if (
    ...
    with res_col1:
      ...
      if st.button(
          "🚀 انشر الآن على فيسبوك", type="primary", use_container_width=True
      ):
        ...
        with st.spinner("🔄 جاري النشر عبر Facebook Graph API..."):
          res = publish_to_facebook_page(
              current_img, current_msg, fb_page_id, fb_page_token
          )
...
with tabs[1]:
  ...
  st.caption(
      "الأداة هتدخل على صفحة المقالات، تختار موضوع جديد غير مكرر ومناسب للناس"
      " العاديين (بمساعدة الذكاء الاصطناعي)، وتنشره تلقائيًا على فيسبوك في الوقت"
      " المحدد كل يوم."
  )

  st.info(
      "⚠️ **ملحوظة مهمة**: الجدولة دي بتشتغل جوه نفس عملية الـ Streamlit، يعني"
      " لازم السيرفر يفضل شغال باستمرار (سواء على جهازك مفتوح، أو على استضافة"
      " سحابية) عشان تشتغل في الميعاد. لو قفلت التطبيق أو أعدت تشغيله، الجدولة"
      " بتتوقف ولازم تبدأها تاني."
  )
  ...
  source_url = st.text_input(
      "🔗 رابط صفحة المقالات (المصدر):", value=DEFAULT_SOURCE_URL
  )

  st.markdown("#### 📅 فترة التشغيل")
  col_a, col_b = st.columns(2)
  with col_a:
    saved_start = load_setting(
        "auto_start_date", datetime.now().strftime("%Y-%m-%d")
    )
    try:
    ...
    start_date = st.date_input(
        "من تاريخ:", value=default_start, min_value=datetime.now().date()
    )
  with col_b:
    saved_end = load_setting(
        "auto_end_date",
